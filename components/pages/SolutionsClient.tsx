...
const PRODUCTS: Record<ProductKey, { name: string; tag: string; accent: string; accentSoft: string; gradient: string; features: { icon: ReactElement; title: string; desc: string }[] }> = {
  biprapay: {
    ...
    features: [
      ...
      { icon: <path d="M23 4v6h-6M1 20v-6h6M3.51 9a9 9 0 0 1 14.85-3.36L23 10M1 14l4.64 4.36A9 9 0 0 0 20.49 15" />, title: "Subscription payments", desc: "Recurring billing on any cycle" },
    ...
    <section className="section"><div className="inner">
      <div style={{display:"flex",flexDirection:"column",gap:"32px"}}>
        {INDUSTRIES.map((ind, i) => {
          const iconBlock = (
            <div>
              <div className="solutions-ind-icon" style={{background: ind.gradient, boxShadow: `0 8px 20px ${ind.accent}48`}}><svg width="24" height="24" viewBox="0 0 24 24" fill="none" stroke="#fff" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">{ind.icon}</svg></div>
              <span className="badge" style={{marginBottom:"16px",background:ind.accentSoft,color:ind.accent}}>{ind.name}</span>
              <h2 style={{fontSize:"28px",fontWeight:"800",letterSpacing:"-.5px",marginBottom:"14px",lineHeight:"1.2"}}>{ind.heading}</h2>
              <p style={{fontSize:"15px",color:"var(--muted)",lineHeight:"1.75",marginBottom:"20px"}}>{ind.description}</p>
              <ul className="flist">
                {ind.features.map((f) => (
                  <li key={f.slug} id={f.slug} className="fitem" style={{scrollMarginTop:"calc(var(--nav) + 24px)"}}><span className="fcheck" style={{background:ind.accentSoft}}><svg width="10" height="10" viewBox="0 0 10 10" fill="none"><polyline points="2,5 4,7 8,3" stroke={ind.accent} strokeWidth="1.8"></polyline></svg></span>{f.title}</li>
                ))}
              </ul>
